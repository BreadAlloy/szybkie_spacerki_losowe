...
struct grafika {
    ...
    void plot_image(std::string nazwa, float skala = 1.0f){
        float x = skala * (float)width;
        float y = skala * (float)height;

        if (ImPlot::BeginPlot(nazwa.c_str(), ImVec2(x, y))) {
            ImPlot::PlotImage("##Obraz", (ImTextureID)(intptr_t)texture
