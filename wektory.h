...
struct statyczny_wektor{ // wektor automatycznie mallocujacy i zwalnijacy, nie ma resize
	...
	__host__ void cuda_malloc(){
		...
		sprawdzCudaErrors(cudaMalloc((void**)&pamiec_device, bajt_rozmiar()));
	}
	...
	__host__ void cuda_zanies(cudaStream_t stream){
		...
		sprawdzCudaErrors(cudaMemcpyAsync((void*)pamiec_device, (void*)pamiec_host, bajt_rozmiar(), cudaMemcpyHostToDevice, stream));
	}

	__host__ void cuda_zanies_do(cudaStream_t stream, typ_wskaznika* tu_zanies) {
		if (rozmiar == 0) return;
		ASSERT_Z_ERROR_MSG(pamiec_host != nullptr, "Nie ma pamieci na hoscie\n");
		sprawdzCudaErrors(cudaMemcpyAsync((void*)tu_zanies, (void*)pamiec_host, bajt_rozmiar(), cudaMemcpyHostToDevice, stream));
	}

	__host__ void cuda_przynies_z(cudaStream_t stream, typ_wskaznika* stad_przynies) {
		if (rozmiar == 0) return;
		ASSERT_Z_ERROR_MSG(pamiec_host != nullptr, "Nie ma pamieci na hoscie\n");
		sprawdzCudaErrors(cudaMemcpyAsync((void*)pamiec_host, (void*)stad_przynies, bajt_rozmiar(), cudaMemcpyDeviceToHost, stream));
	}
	...
	__host__ void cuda_przynies(cudaStream_t stream){
		...
		ASSERT_Z_ERROR_MSG(pamiec_device != nullptr, "Nie ma pamieci na gpu\n");
		ASSERT_Z_ERROR_MSG(pamiec_host != nullptr, "Nie ma pamieci na hoscie\n");
		sprawdzCudaErrors(cudaMemcpyAsync((void*)pamiec_host, (void*)pamiec_device, bajt_rozmiar(), cudaMemcpyDeviceToHost, stream));
	}

	__host__ void cuda_free() {
		if (rozmiar == 0) return;
		ASSERT_Z_ERROR_MSG(pamiec_device != nullptr, "Nic nie ma w pamieci");
		sprawdzCudaErrors(cudaFree((void*)pamiec_device));
