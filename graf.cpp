...
graf graf_lini(uint32_t liczba_wierzcholkow, bool z_nazwami){
	...
	*/
	...
	G.zdefiniuj_liczbe_polaczen(0, 1);
	for (ID_W i = 1; i < liczba_wierzcholkow - 1; i++) {
		G.zdefiniuj_liczbe_polaczen(i, 2);
	}
	G.zdefiniuj_liczbe_polaczen(liczba_wierzcholkow - 1, 1);
...
graf graf_krata_2D(uint32_t liczba_wierzcholkow_boku, bool z_nazwami){
	...
	j   V	  1			   2			2		           2              1
			...
			(...)        (...)  	  (...)              (...)  		(...)
			  |			   |			|				   |			  |
			  1   		   2	        2  				   2              1		  	
		   |0,n-2} 0--0 |1,n-2} 1--0 |2,n-2} 1-(...)-0 |n-2,n-2} 1--0 |n-1,n-2}
			  2			   3			3   			   3 			  3 
			  |			   |            |				   |			  |
			  1		       2     		2	  	 	       2              2
		   |0,n-1} 0--0 |1,n-1} 1--0 |2,n-1} 1-(...)-0 |n-2,n-1} 1--0 |n-1,n-1}
	...
	*/
	...
	for(uint32_t i = 0; i < srodek.liczba_wierzcholkow(); i++){
		srodek.dodaj_polaczenia(i, 2);
	}

	for (uint32_t i = 0; i < gora_dol.liczba_wierzcholkow(); i++) {
